...
from verl.utils.tokenizer import collapse_media_blocks
...
# A vLLM built from a source checkout with no release tag reports the setuptools-scm
# placeholder "0.1.dev1+g<sha>", which makes every version gate take its oldest branch.
# VERL_VLLM_VERSION states the effective version, the override verl/third_party/vllm/__init__.py
# already honours.
...
class vLLMHttpServer:
    ...
    async def generate(
        ...
        prompt_ids = collapse_media_blocks(prompt_ids, self.model_config.processor)
        multi_modal_data = {}
